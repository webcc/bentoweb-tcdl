...
><!-- This version: $Date: 2009-04-29 10:25:44 $ -->
  ...
  <iso:pattern id="root" name="Top Level Test Case Description element">
    <iso:rule context="/*">
  ...
  <iso:pattern id="formalMetadataCreator" name="Formal metadata: dc:creator">
  ...
  <iso:pattern id="formalMetadataLanguage" name="Formal metadata: language">
  ...
  <iso:pattern id="formalMetadataRights" name="Formal metadata: dc:rights">
  ...
  <iso:pattern id="formalMetadataDate" name="Formal metadata: CVS date">
  ...
  <iso:pattern id="formalMetadataStatus_1" name="Formal metadata: status">
  ...
  <iso:pattern id="formalMetadataStatus_2" name="Formal metadata: status">
    <iso:rule context="tcd:formalMetadata/tcd:status">
  ...
  <iso:pattern id="formalMetadataVersion" name="Formal metadata: version">
  ...
  <iso:pattern id="formalMetadataSource" name="Formal metadata: source">
  ...
  <iso:pattern id="technologiesSpec" name="Technologies: technical spec">
  ...
  <iso:pattern id="technologiesMediatype" name="Technologies: MIME type for file types/formats">
  ...
  <iso:pattern id="technologiesHandle" name="Technologies: handle for specifications/formats">
    <iso:rule context="tcd:technicalSpec">
      <iso:assert test="@handle">Each tcd:technicalSpec element must have a handle attribute.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="testCasePreconditions" name="Test case: preconditions.">
  ...
  <iso:pattern id="testCasePurposeWideClaim" name="Purpose: limit claims to WCAG 2.0">
  ...
  <iso:pattern id="testCaseRequiredTests" name="Test case: required tests must not be used.">
    <iso:rule context="tcd:testCase/tcd:requiredTests">
      <iso:assert test="."><iso:name/> must not be used.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="testCaseHTTPRequest" name="Test case: HTTP request">
  ...
  <iso:pattern id="testCaseObsoleteHTTPRequest" name="Test case: obsolete HTTP request">
    <iso:rule context="tcd:testCase/tcd:files/tcd:file/tcd:httpRequest">
      <iso:assert test="."><iso:name/> is obsolete.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="rulesPrimaryOnly" name="Rules: only primary rules allowed.">
  ...
  <iso:pattern id="rulesLocationXPathMappings" name="Rules: XPath expressions in location have namespace mapping">
    ...
    <iso:rule context="tcd:rule/tcd:locations/tcd:location[@xpath][string-length(substring-before(@xpath, ':')) &gt; 0]">
      ...
      <iso:assert test="count(/tcd:testCaseDescription/tcd:namespaceMappings/tcd:namespace[@nsPrefix = substring-before(current()/@xpath, ':')]) &gt; 0">Namespace prefix <iso:value-of select="substring-before(@xpath, ':')"/> is not mapped to a URI.</iso:assert>
      <!-- iso:assert above uses substring-before(current()/@xpath instead of substring-before(@xpath... cf. http://www.biglist.com/lists/xsl-list/archives/200002/msg00481.html : Compare attribute values (16 Feb 2000) -->
    </iso:rule>
    <iso:rule context="tcd:rule/tcd:locations/tcd:location/earl:xPath/earl:expression">
      <!--iso:assert test="/tcd:testCaseDescription/tcd:namespaceMappings[tcd:namespace/@nsPrefix = substring-after(substring-before(., ':'), '/')]"><iso:name/> contains an unbound namespace prefix: <iso:value-of select="substring-after(substring-before(., ':'), '/')" />.</iso:assert-->
    </iso:rule><!-- test="not(contains(normalize-space(.), '/:')) and (count(...))" --><!-- @@todo fix -->
  ...
  <iso:pattern id="rulesLocationEarlPointers" name="Rules: check use of EARL Pointers">
    <!-- @@synchronize with EARL 1.0 location pointers : http://www.w3.org/TR/EARL10-Schema/#instancelocation -->
  </iso:pattern>

  <iso:pattern id="rulesLocationTechRefsExist" name="Rules: check that @techrefs is used.">
    <iso:rule context="tcd:rule/tcd:locations/tcd:location">
      <iso:assert test="@techrefs"><iso:name/>/@techrefs is required.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="rulesLocationTechRefsSingle" name="Rules: check reference (assuming just one) from locations to techniques/failures">
    <iso:rule context="tcd:rule/tcd:locations/tcd:location[@techrefs][not(contains(@techrefs, ' '))]">
      <iso:assert test="count(../../tcd:techniques/tcd:technique[@id = current()/@techrefs]) = 1">There is no technique element with ID <iso:value-of select="./@techrefs"/>.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="rulesLocationTechRefsMultiple" name="Rules: check references from locations to techniques/failures">
    <iso:rule context="tcd:rule/tcd:locations/tcd:location[@techrefs][contains(current()/@techrefs, ' ')]">
      ...
      <iso:assert test="count(../../../tcd:rule/tcd:techniques/tcd:technique[@id = substring-before(current()/@techrefs, ' ')]) = 1">There is no technique element with ID <iso:value-of select="substring-before(./@techrefs, ' ')"/>. (Other references not checked.)</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="rulesLocationTechniques" name="Rules: references to techniques/failures are required">
  ...
  <iso:pattern id="rulesMostRecentGuidelines" name="Rules: references to success criteria must point to most recent draft">
  ...
  <iso:pattern id="rulesMostRecentTechniques" name="Rules: references to techniques/failures must point to most recent draft">
  ...
  <iso:pattern id="rulesOnlyOneTechnique" name="Rules: one techniques or failure allowed">
  ...
  <iso:pattern id="rulesNoFunctionalOutcome" name="Rules: functionalOutcome element must not be used">
    <iso:rule context="tcd:rule/tcd:functionalOutcome">
      <iso:assert test="."><iso:name/> must not be used.</iso:assert>
    </iso:rule>
  </iso:pattern>

  <iso:pattern id="emptyNameSpacePrefix" name="Namespace mapping: don't use an empty namespace prefix">
