...
/>
  ...
  <xsl:template name="getTestTitle">Test <xsl:value-of select="$batchPosition"/> van <xsl:value-of select="$batchTotal"/></xsl:template>

  <xsl:template name="getInstructionsTitle">Instructies:</xsl:template>

  <xsl:template name="getTestCaseString">Testgeval:</xsl:template>

  <xsl:template name="getQuestionString">Vraag:</xsl:template>

  <xsl:template name="getYes">Ja</xsl:template>
  <xsl:template name="getNo">Nee</xsl:template>

  <xsl:template name="getSelectFileText">
    <xsl:param name="testFileCount" />
        <xsl:if test="$testFileCount &gt; 1">
          <p>Bekijk de volgende pagina's:</p>
        </xsl:if>
        <xsl:if test="$testFileCount = 1">
          Bekijk de volgende
        </xsl:if>
        <xsl:if test="$testFileCount = 0">
          <strong class="debug">Ontbrekende testpagina!</strong>
        </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="printTestFileWithNumber">
    <xsl:param name="testFilePosition" />
        <xsl:if test="$testFilePosition &gt;  0">Testpagina <xsl:value-of select="$testFilePosition" /></xsl:if>
        <xsl:if test="$testFilePosition = 0"><strong class="debug">Ontbrekende testpagina!</strong></xsl:if>
  </xsl:template>
  ...
  <xsl:template name="getTextAreaLabel">
      Geef alstublieft uw commentaar:
  </xsl:template>


  <!-- Text for submit button (localised) -->
  <xsl:template name="getSubmitButtonText">Verzenden en naar volgende test</xsl:template>

  <!-- Text for comment button (localised) -->
  <xsl:template name="getCommentButtonText">Commentaar</xsl:template>
  
  <!-- Text for exit button (localised) -->
  <xsl:template name="getExitButtonText">Afsluiten</xsl:template>
