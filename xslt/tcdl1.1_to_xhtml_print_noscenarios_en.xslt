...
>
<xsl:include href="tcdl1.1_to_xhtml_print_noscenarios.xslt"/>
...
/>
  ...
  <xsl:template name="getTestMode">
    <xsl:param name="mode" />
    <!--xsl:if test="starts-with($mode, 'automatic')">Automatic evaluation.</xsl:if>
    <xsl:if test="starts-with($mode, 'enduser')">End user.</xsl:if>
    <xsl:if test="starts-with($mode, 'experts')">Three or more accessibility experts.</xsl:if>
    <xsl:if test="starts-with($mode, 'oneExpert')">Accessibility expert.</xsl:if-->
    <xsl:choose>
      <xsl:when test="starts-with($mode, 'semi-automatic')">Semi-automatic evaluation.</xsl:when>
      <xsl:when test="starts-with($mode, 'automatic')">Automatic evaluation.</xsl:when>
      <xsl:when test="starts-with($mode, 'enduser')">End user.</xsl:when>
      <xsl:when test="starts-with($mode, 'experts')">Three or more accessibility experts.</xsl:when>
      <xsl:when test="starts-with($mode, 'oneExpert')">Accessibility expert.</xsl:when>
      <xsl:otherwise>Test mode unknown (<xsl:value-of select="$mode" />).</xsl:otherwise>
    </xsl:choose>
  </xsl:template>
